...
function analyticspro_wfs_open_cache_db(): SQLite3
{
    ...
    $db->exec('CREATE TABLE IF NOT EXISTS particelle_cache (
        ...
        lng REAL NOT NULL,
        area_mq REAL,
        source VARCHAR(20) NOT NULL DEFAULT \'WFS-AdE\',
    ...
    // Migration 003: add source column to existing databases that pre-date this column.
    $cols = $db->query('PRAGMA table_info(particelle_cache)');
    $hasSource = false;
    if ($cols) {
    ...
    if (!$hasSource) {
        $db->exec("ALTER TABLE particelle_cache ADD COLUMN source VARCHAR(20) NOT NULL DEFAULT 'WFS-AdE'");
    }
...
/**
 * Saves WFS/Zornade result to the local SQLite cache.
 */
function analyticspro_wfs_save_cached_particella(SQLite3 $db, string $codCatastale, string $foglio, string $particella, array $data): void
{
    ...
    $stmt->bindValue(':source', (string) ($data['source'] ?? 'WFS-AdE'), SQLITE3_TEXT);
    $stmt->execute();
}
